 ...
 * 2) Bassam Yasser Mohamed Hassan, ID: 20230089,Filters taken: Gray scale, Merge, Resize, Sunlight
    ...
    if (width1 == width2 && height1 == height2) {
        ...
        for (int i = 0; i < width1; ++i) {
            for (int j = 0; j < height1; ++j) {
                for (int k = 0; k < 3; ++k) {
                    image1(i, j, k) = 0.5 * image2(i, j, k) + 0.5 * image1(i, j, k);
                    if (image1(i, j, k) > 255) {
                        image1(i, j, k) = 255;
                    }
                }
    ...
    if (width1 > width2 && height1 > height2) {
        ...
        for (int i = 0; i < width2; ++i) {
            for (int j = 0; j < height2; ++j) {
                for (int k = 0; k < 3; ++k) {
                    image2(i, j, k) = 0.5 * image2(i, j, k) + 0.5 * image1(floor(i * r1), floor(j * r2), k);
                    if (image2(i, j, k) > 255) {
                        image2(i, j, k) = 255;
                    }
                }
        ...
        if (choice == 1) {
            ...
            if (width1 > width2 && height2 > height1) {
                Image image1resized(width1, height2);
                Image image2resized(width1, height2);

                double r1i1 = static_cast<double>(width1) / width1;
                double r2i1 = static_cast<double>(height1) / height2;

                for (int i = 0; i < width1; ++i) {
                    for (int j = 0; j < height2; ++j) {
                        for (int k = 0; k < 3; ++k) {
                            image1resized(i, j, k) = image1(floor(i * r1i1), floor(j * r2i1), k);
                        }
                    }
                }


                double r1i2 = static_cast<double>(width2) / width1;
                double r2i2 = static_cast<double>(height2) / height2;

                for (int i = 0; i < width1; ++i) {
                    for (int j = 0; j < height2; ++j) {
                        for (int k = 0; k < 3; ++k) {
                            image2resized(i, j, k) = image2(floor(i * r1i2), floor(j * r2i2), k);
                        }
                    }
                }
                for (int i = 0; i < width1; ++i) {
                    for (int j = 0; j < height2; ++j) {
                        for (int k = 0; k < 3; ++k) {
                            image1resized(i, j, k) = 0.5 * image2resized(i, j, k) + 0.5 * image1resized(i, j, k);
                            if (image1resized(i, j, k) > 255) {
                                image1resized(i, j, k) = 255;
                            }
                        }
                    }
                }
                image1 = image1resized;
            } else if (width2 > width1 && height1 > height2) {
                Image image1resized(width2, height1);
                Image image2resized(width2, height1);

                double r1i1 = static_cast<double>(width1) / width2;
                double r2i1 = static_cast<double>(height1) / height1;
                ...
                double r1i2 = static_cast<double>(width2) / width2;
                double r2i2 = static_cast<double>(height2) / height1;
            ...
            Image image1cropped(min(width1, width2), min(height1, height2));


            for (int i = 0; i < min(width1, width2); i++) {
                for (int j = 0; j < min(height1, height2); j++) {
                    for (int k = 0; k < 3; k++) {
                        image1cropped(i, j, k) = image1(i, j, k);
                    }
                }
            }

            Image image2cropped(min(width1, width2), min(height1, height2));


            for (int i = 0; i < min(width1, width2); i++) {
                for (int j = 0; j < min(height1, height2); j++) {
                    for (int k = 0; k < 3; k++) {
                        image2cropped(i, j, k) = image2(i, j, k);
                    }
                }
            }
            ...
            for (int i = 0; i < min(width1, width2); ++i) {
                for (int j = 0; j < min(height1, height2); ++j) {
                    for (int k = 0; k < 3; ++k) {
                        image1cropped(i, j, k) = 0.5 * image2cropped(i, j, k) + 0.5 * image1cropped(i, j, k);
                        if (image1cropped(i, j, k) > 255) {
                            image1cropped(i, j, k) = 255;
                        }
                    }
                }
            }
            image1 = image1cropped;
...
void sunlight_image(Image &image){
    for (int i = 0; i < image.width; ++i) {
        for (int j = 0; j < image.height; ++j) {
            image(i,j,2) *= 0.75;
        }
